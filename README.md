...
- 🕒 **自动化监控**：利用 GitHub Actions 定时（默认每小时）检查应用价格。
- 💰 **价格变动通知**：支持邮件 (SMTP) 和 Webhook (飞书/钉钉/Slack/Telegram) 通知。
- 📊 **历史记录**：自动记录应用价格信息到 `data/history.json`。
...
价格数据存储在 `data/history.json` 中。每次检测完成后，如果有数据更新，GitHub Actions 会自动提交更改回仓库。
